/**
 * Chemistry Quiz Component
 * Handles quiz generation, display, and scoring using static data
 */

class ChemistryQuiz {
    constructor() {
        this.quizData = typeof CHEMISTRY_QUIZ_DATA !== 'undefined' ? CHEMISTRY_QUIZ_DATA : [];
    ...
    displayQuizUI() {
                ...
                <div class="quiz-options" style="display: flex; flex-direction: column; gap: 8px;">
                    ${q.options.map((option, optIdx) => `
                        ...
                        " onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="if (!this.querySelector('input').checked) this.style.background='rgba(255,255,255,0.05)'">
                            <input type="radio" name="chem-quiz-q-${qIdx}" value="${optIdx}" style="margin: 0; accent-color: #8b5cf6;">
                            <span>${option}</span>
                        </label>
                    `).join('')}
                </div>
        ...
        const content = `
            ...
            <button id="chem-quiz-submit" style="
                width: 100%;
                padding: 14px;
        ...
        this.showModal('Chemistry Knowledge Check', content, (modal) => {
            // Track answers
            modal.querySelectorAll('input[type="radio"]').forEach(radio => {
                radio.addEventListener('change', (e) => {
                    ...
                    e.target.parentElement.style.background = 'rgba(139, 92, 246, 0.15)';
                    e.target.parentElement.style.borderColor = 'rgba(139, 92, 246, 0.5)';
                });
            });

            const submitBtn = modal.querySelector('#chem-quiz-submit');
    ...
    showResults(score, results) {
        ...
        const content = `
            <div style="text-align: center; padding: 24px; background: linear-gradient(135deg, rgba(139,92,246,0.1) 0%, rgba(168,85,247,0.1) 100%); border-radius: 12px; margin-bottom: 20px; border: 1px solid rgba(139,92,246,0.2);">
        ...
        this.showModal('Quiz Results', content, (modal) => {
            modal.querySelector('.modal-close-btn-bottom').addEventListener('click', () => {
                const modalEl = document.getElementById('chem-quiz-modal');
    ...
    showModal(title, content, onShow) {
        // Remove existing
        const existing = document.getElementById('chem-quiz-modal');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'chem-quiz-modal';
        modal.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
