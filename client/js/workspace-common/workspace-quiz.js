class WorkspaceQuiz {
    constructor(options = {}) {
        this.workspace = options.workspace || 'chemistry';
        this.containerId = options.containerId || 'navbar-controls';
        this.dataVarName = `${this.workspace.toUpperCase()}_QUIZ_DATA`;
        this.quizData = window[this.dataVarName] || [];
    ...
    addButton() {
        ...
        if (container.querySelector('.quiz-btn')) return;

        const btn = document.createElement('button');
        btn.className = 'quiz-btn';
        btn.innerHTML = '<span class="material-icons-round">quiz</span>';
        btn.title = `Take ${this.capitalize(this.workspace)} Quiz`;

        // Standard styling to match other controls
        btn.style.cssText = `
    ...
    startQuiz() {
        if (!this.quizData || this.quizData.length === 0) {
            console.warn(`Quiz data for ${this.workspace} not loaded. Checked window.${this.dataVarName}`);
            alert('Quiz data is currently unavailable.');
            return;
        }
    ...
    displayQuizUI() {
        // Reuse the UI logic from ChemistryQuiz but adapted for generic use
                ...
                <div class="quiz-options" style="display: flex; flex-direction: column; gap: 8px;">
                    ${q.options.map((option, optIdx) => `
                        ...
                        " onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="if (!this.querySelector('input').checked) this.style.background='rgba(255,255,255,0.05)'">
                            <input type="radio" name="ws-quiz-q-${qIdx}" value="${optIdx}" style="margin: 0; accent-color: var(--accent-primary);">
                            <span>${option}</span>
                        </label>
                    `).join('')}
                </div>
        ...
        const content = `
            ...
            <button id="ws-quiz-submit" style="
                width: 100%;
                padding: 14px;
        ...
        this.showModal(`${this.capitalize(this.workspace)} Knowledge Check`, content, (modal) => {
            modal.querySelectorAll('input[type="radio"]').forEach(radio => {
                radio.addEventListener('change', (e) => {
                    ...
                    e.target.parentElement.style.background = 'var(--bg-elevated)'; // Approximation
                    e.target.parentElement.style.background = 'rgba(255,255,255,0.1)';
                    e.target.parentElement.style.borderColor = 'var(--accent-primary)';
                });
            });

            const submitBtn = modal.querySelector('#ws-quiz-submit');
    ...
    submitQuiz() {
        let correctCount = 0;
        let answeredCount = 0;
        this.userAnswers.forEach(ans => { if (ans !== null) answeredCount++; });
    ...
    showResults(score, results) {
        ...
        const content = `
            <div style="text-align: center; padding: 24px; background: linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.1) 100%); border-radius: 12px; margin-bottom: 20px; border: 1px solid var(--accent-primary);">
            ...
            <div style="max-height: 400px; overflow-y: auto; padding-right: 8px;">${resultHTML}</div>
            <button class="modal-close-btn-bottom" style="width: 100%; padding: 12px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; color: white; cursor: pointer; margin-top: 16px; transition: all 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='rgba(255,255,255,0.1)'">Close</button>
        `;

        this.showModal('Quiz Results', content, (modal) => {
            modal.querySelector('.modal-close-btn-bottom').addEventListener('click', () => {
                const modalEl = document.getElementById('ws-quiz-modal');
    ...
    showModal(title, content, onShow) {
        const existing = document.getElementById('ws-quiz-modal');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'ws-quiz-modal';
        modal.style.cssText = `
        ...
        modal.innerHTML = `
            <div class="modal-content" style="
                background: #0f1623; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px;
                width: 90%; max-width: 600px; max-height: 85vh; display: flex; flex-direction: column;
                box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); animation: slideUp 0.3s;
            ">
                <div class="modal-header" style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <button class="modal-close" style="background: none; border: none; color: rgba(255,255,255,0.5); font-size: 24px; cursor: pointer; padding: 4px; border-radius: 4px; display: flex; align-items: center;" onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="this.style.background='none'">&times;</button>
                </div>
                <div class="modal-body" style="padding: 24px; overflow-y: auto;">${content}</div>
            </div>
        `;
        ...
        if (!document.getElementById('ws-quiz-styles')) {
            const style = document.createElement('style');
            style.id = 'ws-quiz-styles';
