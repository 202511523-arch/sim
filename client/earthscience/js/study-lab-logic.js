...
let drawSettings = {
    tool: 'pen',
    color: '#60a5fa', // Earth Science default accent
    brushSize: 3,
    opacity: 100
};
...
const WorkspaceState = {
    ...
    init() {
        ...
        if (path.includes('constellation.html')) this.currentPage = 'constellation';
        else if (path.includes('detailearth.html')) this.currentPage = 'detail-earth';
        else if (path.includes('tectonicplates.html')) this.currentPage = 'tectonic';
        else if (path.includes('index.html')) this.currentPage = 'solar-system';
        else this.currentPage = 'study';

        console.log(`Workspace initialized for Earth Science - ${this.currentPage}`);
        ...
        try {
            ...
            if (this.projectId && this.projectId !== 'dev-demo-project') {
                ...
                const res = await fetch(`/api/projects/${this.projectId}/workspace-state`, {
                    ...
                    credentials: 'include',
                    body: JSON.stringify({
                        workspace: `earth-science-${this.currentPage}`,
                        state
                    })
                });
    ...
    saveToLocalStorage(stateData = null) {
        try {
            ...
            const key = `earth-science-${this.currentPage}-state-${this.projectId || 'demo'}`;
        ...
        try {
            if (this.projectId && this.projectId !== 'dev-demo-project') {
                ...
                const res = await fetch(
                    `/api/projects/${this.projectId}/workspace-state?workspace=earth-science-${this.currentPage}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        },
...
// Setup real-time collaboration listeners immediately
...
function setupDrawingCollaboration() {
    const checkCollab = setInterval(() => {
        if (window.workspaceCollaboration) {
            ...
            // Ensure canvas is initialized for viewing (passive mode)
            if (!drawCanvas) {
                drawCanvas = document.getElementById('draw-canvas');
                if (drawCanvas) {
            ...
            console.log('🎨 Drawing real-time collaboration enabled (Earth Science)');
        }
    }, 500);
...
function stopDrawing() {
    ...
    isDrawing = false;
    currentStrokePoints = []; // Reset points
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (visibilityBtn) {
        visibilityBtn.addEventListener('click', () => {
            isCanvasVisible = !isCanvasVisible;
            if (drawCanvas) drawCanvas.style.display = isCanvasVisible ? 'block' : 'none';
            visibilityBtn.textContent = isCanvasVisible ? 'Hide' : 'Show';
            visibilityBtn.style.color = isCanvasVisible ? 'var(--accent)' : 'var(--text-secondary)';
        });
    }
