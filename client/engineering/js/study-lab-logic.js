...
let drawSettings = {
    tool: 'pen',
    color: '#fbbf24', // Mechanical Engineering default accent
    brushSize: 3,
    opacity: 100
};
...
const WorkspaceState = {
    ...
    init() {
        ...
        if (path.includes('index.html')) this.currentPage = 'mechanical-main';
        else this.currentPage = 'study';

        console.log(`Workspace initialized for Engineering - ${this.currentPage}`);
        ...
        try {
            ...
            if (this.projectId && this.projectId !== 'dev-demo-project') {
                ...
                const res = await fetch(`/api/projects/${this.projectId}/workspace-state`, {
                    ...
                    credentials: 'include',
                    body: JSON.stringify({
                        workspace: `mechanical-${this.currentPage}`,
                        state
                    })
                });
    ...
    saveToLocalStorage(stateData = null) {
        try {
            ...
            const key = `mechanical-${this.currentPage}-state-${this.projectId || 'demo'}`;
        ...
        try {
            if (this.projectId && this.projectId !== 'dev-demo-project') {
                ...
                const res = await fetch(
                    `/api/projects/${this.projectId}/workspace-state?workspace=mechanical-${this.currentPage}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        },
